...
open class CustomRegionBasedStorage internal constructor(
    ...
    fun getBlockEntities(chunkPos: ChunkPos): List<BlockEntity> {
        ...
        val blockEntitiesList = nbt.getList("block_entities").orElse(null) ?: return emptyList()
        
        return blockEntitiesList.filterIsInstance<NbtCompound>().mapNotNull { compoundTag ->
            val blockPos = BlockPos(
                compoundTag.getInt("x").orElse(0),
                compoundTag.getInt("y").orElse(0),
                compoundTag.getInt("z").orElse(0)
            )
            val blockStateIdentifier = Identifier.of(compoundTag.getString("id").orElse(""))
            ...
            runCatching {
                ...
                Registries.BLOCK_ENTITY_TYPE
                    ...
                    ?.instantiate(blockPos, block.defaultState)?.apply {
                        val readView = NbtReadView.create(ErrorReporter.EMPTY, world.registryManager, compoundTag)
                        read(readView)
                    }
            }.getOrNull()
        }
