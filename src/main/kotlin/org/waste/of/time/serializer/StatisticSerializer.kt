...
import org.waste.of.time.mixin.StatHandlerAccessor
...
object StatisticSerializer {
    fun writeStats() {
        ...
        val completeStatMap = (WorldTools.mc.player?.statHandler as? StatHandlerAccessor)?.statMap ?: return
        ...
        CoroutineScope(Dispatchers.IO).launch {
            WorldTools.withSessionBlocking {
                ...
                if (statDirectory.toFile().exists().not()) {
                ...
                path.writeText(JsonObject().apply {
                    ...
                    add("stats", JsonObject().apply {
                        completeStatMap.toMap().entries.groupBy { it.key.type }.forEach { (type, entries) ->
                ...
                WorldTools.LOGGER.info("Saved stats.")
            }
        }
    }
}